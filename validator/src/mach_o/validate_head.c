...
t_validator_error	validate_head_common(t_macho_file *obj)
{
	if (!in(&obj->head.cputype, (t_memory_descriptor){4, 14, 4, 0},
			(uint32_t [14]){CPU_TYPE_VAX, CPU_TYPE_MC680x0, CPU_TYPE_X86,
	...
	if (!in(&obj->head.filetype, (t_memory_descriptor){4, 11, 4, 0},
			(uint32_t [11]){MH_OBJECT, MH_EXECUTE, MH_FVMLIB, MH_CORE,
			MH_PRELOAD, MH_DYLIB, MH_DYLINKER, MH_BUNDLE, MH_DYLIB_STUB,
			MH_DSYM, MH_KEXT_BUNDLE}, 0))
...
{
	const t_struct_descriptor	sd_head_32 = {4, sizeof(struct mach_header),
...
{
	const t_struct_descriptor	sd_head_64 = {4, sizeof(struct mach_header),
		{{4, 1, 4, 0}, {sizeof(cpu_type_t), 1, sizeof(cpu_type_t), 1},
